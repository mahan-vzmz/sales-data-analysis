"""End-to-end sales data cleaning, analysis, and Excel export.

Run from the project root with:

    python src/sales_analysis.py

The script never modifies the raw workbook. It recreates the cleaned dataset
and analytical report from scratch so the project is reproducible.
"""
...
def analyze_sales(data: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    summary = pd.DataFrame(
        [
            ...
            ("Effective Discount Rate (%)", round(total_discount / total_gross * 100, 2)),
            ...
            ("Most-Used Payment Method", payment.sort_values("Orders", ascending=False).iloc[0]["Payment_Method"]),
        ],
...
def export_cleaned_workbook(
    ...
    with pd.ExcelWriter(PROCESSED_DATA_PATH, engine="openpyxl") as writer:
...
def main() -> None:
    ...
    export_cleaned_workbook(clean_df, cleaning_log)

    # The analytical report deliberately reads the persisted cleaned workbook.
    # This keeps the automated script identical to the notebook workflow and
    # prevents tiny binary-float differences at Excel serialization boundaries.
    persisted_clean_df = pd.read_excel(
        PROCESSED_DATA_PATH,
    ...
    export_report(analysis_df, analyses)
    ...
    print(f"Unique orders: {clean_df['Order_ID'].nunique():,}")
    print(f"Units sold: {int(clean_df['Quantity'].sum()):,}")
    print(f"Net revenue: ${float(summary['Total Net Revenue (USD)']):,.2f}")
    print(f"Cleaned workbook: {PROCESSED_DATA_PATH}")
    print(f"Analysis workbook: {REPORT_PATH}")
